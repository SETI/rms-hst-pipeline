...
class TestMultiversionBundleFS(unittest.TestCase):
    ...
    def test_make_lidvid_directories(self):
        self.fs.make_lidvid_directories(LIDVID("urn:nasa:pds:b::3.0"))
        self.assertTrue(self.fs.exists(u"/b/v$3.0"))
        self.assertTrue(self.fs.exists(u"/b/v$3.0/subdir$versions.txt"))
        # arbitrarily set the text (to something illegal)
        self.fs.settext(u"/b/v$3.0/subdir$versions.txt", u"placeholder")
        # try rerunning
        self.fs.make_lidvid_directories(LIDVID("urn:nasa:pds:b::3.0"))
        # make sure we didn't overwrite
        self.assertEqual(
            u"placeholder", self.fs.gettext(u"/b/v$3.0/subdir$versions.txt")
        )

        self.fs.make_lidvid_directories(LIDVID("urn:nasa:pds:b:c:p::1.1"))
        # make sure we didn't create a subdir$versions file, since products
        # don't have subdirs.
        self.assertTrue(self.fs.exists(u"/b/c/p/v$1.1"))
        self.assertFalse(self.fs.exists(u"/b/c/p/v$1.1/subdir$versions.txt"))
