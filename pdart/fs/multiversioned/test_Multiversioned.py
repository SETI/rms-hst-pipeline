...
def is_new(lidvid, contents, mv):
    # type: (LIDVID, VersionContents, Multiversioned) -> bool
    return mv[lidvid] != contents


class Test_Multiversioned(unittest.TestCase):
    def test_lid_path(self):
        # type: () -> None
        self.assertEqual(u"/b", lid_path(LID("urn:nasa:pds:b")))
        self.assertEqual(u"/b/c", lid_path(LID("urn:nasa:pds:b:c")))
        self.assertEqual(u"/b/c/p", lid_path(LID("urn:nasa:pds:b:c:p")))

    def test_lidvid_path(self):
        # type: () -> None
        self.assertEqual(u"/b/v$1.5", lidvid_path(LIDVID("urn:nasa:pds:b::1.5")))
        self.assertEqual(u"/b/c/v$2.5", lidvid_path(LIDVID("urn:nasa:pds:b:c::2.5")))
        self.assertEqual(
            u"/b/c/p/v$333.123", lidvid_path(LIDVID("urn:nasa:pds:b:c:p::333.123"))
        )
    ...
    def test_get_set_items(self):
        # type: () -> None
        no_lidvids = set()  # type: Set[LIDVID]
        ...
        single_file_path = u"/down/a/lot/of/dirs/text.txt"
        single_file_fs.makedirs(fs.path.dirname(single_file_path), None, True)
        single_file_fs.writetext(single_file_path, u"Hello, there!")
    ...
    def test_update_from_single_version(self):
        # type: () -> None
        fs = MemoryFS()
        ...
        def create_collection(bundle_lid, c):
            # type: (LID, str) -> LIDVID
        ...
        self.assertEqual(u"file2", fs.readtext(u"b/c2/p1/v$1.0/dir1/file2.txt"))
        self.assertEqual(u"xxxx", fs.readtext(u"b/c2/p1/v$2.0/dir1/file2.txt"))

        # unchanged files are unchanged
        self.assertEqual(u"file1", fs.readtext(u"b/c2/p1/v$1.0/dir1/file1.txt"))
        self.assertEqual(u"file1", fs.readtext(u"b/c2/p1/v$2.0/dir1/file1.txt"))
