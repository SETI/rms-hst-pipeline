...
def _build_browse_collection(db, browse_deltas, bundle_segment,
                             collection_segment, bundle_path):
    ...
    browse_collection_lid = LIDVID(
        data_collection_lidvid).lid().to_browse_lid()
    collection_path = u'%s%s$/' % (bundle_path, collection_segment)
    browse_collection_path = u'%s%s$/' % \
        (bundle_path, browse_collection_lid.collection_id)
    ...
    db.create_non_document_collection(
    ...
    for product_segment in product_segments:
        ...
        for fits_file in browse_deltas.listdir(product_path):
            ...
            db.create_browse_file(browse_os_filepath,
                                  browse_file,
    ...
    with make_osfs(archive_dir) as archive_osfs, \
        ...
        for collection_segment in collection_segments:
            if collection_segment.startswith('data_'):
                _build_browse_collection(db, browse_deltas,
                                         bundle_segment, collection_segment,
                                         bundle_path)
