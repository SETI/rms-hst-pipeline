...
def _populate_from_document_collection(db,
                                       sv_deltas,
                                       bundle_lidvid,
                                       collection_lidvid,
                                       product_path):
    ...
    for basename in sv_deltas.listdir(product_path):
        sys_filepath = sv_deltas.getsyspath(fs.path.join(product_path,
                                                         basename))
        db.create_document_file(sys_filepath, basename, product_lidvid)

def _populate_from_non_document_collection(db,
                                           sv_deltas,
                                           bundle_lidvid,
                                           collection_lidvid,
                                           collection_path):
    ...
    for product_segment in product_segments:
        ...
        product_lidvid = _extend_initial_lidvid(collection_lidvid,
                                                product_segment)
...
def populate_database(bundle_segment,
    ...
    with make_osfs(archive_dir) as archive_osfs, \
        ...
        for collection_segment in collection_segments:
            ...
            collection_lidvid = _extend_initial_lidvid(bundle_lidvid,
                                                       collection_segment)
            if is_document_collection:
                product_path = u'%s%s$/' % (collection_path, 'phase2')
                _populate_from_document_collection(db,
                                                   sv_deltas,
                                                   bundle_lidvid,
                                                   collection_lidvid,
                                                   product_path)
            else:
                _populate_from_non_document_collection(db,
                                                       sv_deltas,
                                                       bundle_lidvid,
                                                       collection_lidvid,
                                                       collection_path)
