...
class Test_Utils(unittest.TestCase):
    ...
    def test_make_sv_deltas(self):
        with make_sv_osfs(join(self.temp_dir, u'foo')) as base_fs, \
                make_sv_deltas(base_fs,
                               join(self.temp_dir, u'bar')) as deltas_fs:
            names = OSFS(self.temp_dir).walk.dirs(u'/')
            self.assertEquals(
                {u'/foo-sv', u'/bar-deltas-sv',
                 u'/bar-deltas-sv/additions',
                 u'/bar-deltas-sv/deletions'}, set(names))

    def test_make_mv_deltas(self):
        with make_mv_osfs(join(self.temp_dir, u'foo')) as base_fs, \
                make_mv_deltas(base_fs,
                               join(self.temp_dir, u'bar')) as deltas_fs:
            names = OSFS(self.temp_dir).walk.dirs(u'/')
            self.assertEquals(
                {u'/foo-mv', u'/bar-deltas-mv',
                 u'/bar-deltas-mv/additions',
                 u'/bar-deltas-mv/deletions'}, set(names))
    ...
    def test_make_version_view(self):
        with make_mv_osfs(join(self.temp_dir, u'foo')) as base_fs:
            ...
            self.assertEquals({u'/foo-mv', u'/foo-mv/b', u'/foo-mv/b/v$1.0'},
                              set(names))
