...
def make_checksum_manifest(bundle_db, lidvid_to_filepath):
    ...
    sorted_pairs = sorted(make_checksum_pair(file, lidvid_to_filepath)
                          for file in files)
    return ''.join('%s  %s\n' % (hash, path)
                   for (path, hash) in sorted_pairs)


def make_checksum_pair(file, lidvid_to_filepath):
    # type: (File, Callable[[LIDVID], unicode]) -> Tuple[unicode, str]
    def file_to_filepath(file):
        # type: (File) -> unicode
        return fs.path.join(lidvid_to_filepath(LIDVID(file.product_lidvid)),
                            file.basename)
