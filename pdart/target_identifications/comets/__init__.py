...
# must be internally consistent, all referring to the same comet. This can be
...
# inconsistent.
#
# The reason comet_identifications returns a list is for possible cases of
# associated objects, such as fragments of a single comet, that might all
# simultaneously be targets of a given observation.
#
...
#
# For comets that have been re-designated to be minor planets,
# comet_identifications returns the minor planet information, but with cometary
# names included in the list of alt_designations.
#
...
# Information from these pages is saved in files PDS_COMETS_TXT.py and
# SSD_COMETS_CSV.py. It will occasionally be necessary to update these two files
# as new comets are discovered, renamed, reclassified, or are assigned numeric
# NAIF IDs.
################################################################################
...
# INFORMATION REPAIRS
#
# These definitions may need to be modified as new comets are added or comets
# are reclassified.
################################################################################
...
class CometInfo(object):
    ...
    def __eq__(self, other, ignore_suffix=False):
        # ignore_suffix=True identifies comets as equal even if they refer to
        # different fragments
...
# COMETS_BY_NAME is a dictionary indexed by the name of the discoverer(s). Each
# entry is a list of comets with the same discoverer. Each unique comet in this
...
for (name, comet_list) in comet_list_by_name.items():
    ...
    if len(unique_comets) > 1:
        ...
        comet.index = 0
        # This happens if a PDS comet has an index of one, but the name is
...
for comet in COMETS:
    ...
    for name in full_names:
        if name in COMET_LOOKUP:
            # This indicates that one of the names for two different CometInfo
            # objects is not unique. The ideal response is to update the
            # algorithm used by full_names() so that it does not return this
...
# A name followed by "+" returns a list of all the comets associated with that
# discoverer or team
...
def append_comet_designations(info, comet):
    ...
    Needed  to include cometary designations for hybrid objects that are also
    minor planets.
    ...
    # Right now, every identification tuple from the minor_planets module has
    # unit length, so this loop might not be strictly necessary.
    new_info = []
