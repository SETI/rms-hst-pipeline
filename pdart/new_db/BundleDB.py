...
class BundleDB(object):
    ...
    def create_collection(self, collection_lidvid, bundle_lidvid):
        # type: (str, str) -> None
        '''
        Create a collection with this LIDVID if none exists.
        '''
        LIDVID(collection_lidvid)
        if not self.collection_exists(collection_lidvid):
            self.session.add(Collection(lidvid=collection_lidvid,
                                        bundle_lidvid=bundle_lidvid))

    def create_product(self, product_lidvid, collection_lidvid):
        # type: (str, str) -> None
        '''
        Create a product with this LIDVID if none exists.
        '''
        LIDVID(product_lidvid)
        if not self.product_exists(product_lidvid):
            self.session.add(Product(lidvid=product_lidvid,
