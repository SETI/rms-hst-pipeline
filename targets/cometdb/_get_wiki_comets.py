...
def _get_wiki_comet_list(
    ...
    if update:
        ...
        request = requests.get(url, headers=_WIKI_HEADERS)
...
def _get_wiki_numbered_comets(
    ...
    if update:
        logger and logger.info(f'Reading {_WIKI_NUMBERED_URL}')
        request = requests.get(_WIKI_NUMBERED_URL, headers=_WIKI_HEADERS)
        request.raise_for_status()
        html = request.content
    else:
        html = _read_content(basename, logger)
...
def _get_wiki_interstellar_comets(
    ...
    if update:
        logger and logger.info(f'Reading {_WIKI_INTERSTELLAR_URL}')
        request = requests.get(_WIKI_INTERSTELLAR_URL, headers=_WIKI_HEADERS)
        request.raise_for_status()
        html = request.content
    else:
        html = _read_content(basename, logger)
    ...
    for name in names:
        ...
        desigs, elements = mpc_query_by_name(comet['prefix'], logger=logger)
