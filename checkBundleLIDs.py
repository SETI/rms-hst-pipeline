...
for o, a in opts:
    ...
    elif o in ("-i", "--ignore"): ignoreOld = True
...
def flattenFilesOrDirs(paths):
    toReturn = []
    for p in paths:
        if (re.match("\.", p)       #allow initial ./xxx
            and p is not "." and not re.match("\.\/",p)
            and p is not ".." and not re.match("\.\.\/",p)): continue
        if (os.path.isdir(p)):
            for inFile in os.listdir(p):
                if (re.match("\.", inFile)): continue    # skip .DS_Store
                x = flattenFilesOrDirs([p + "/" + inFile])
                if x: toReturn.extend(x)
        else: toReturn.append(p)
    return toReturn
...
def parseFiles(allFiles):
	...
	if not re.search("\.xml$", aFile): continue
	tree = ET.parse(aFile)
	doc = tree.getroot()
	...
	lid = doc.find(tagLid).text
        ...
        if lid in lid2viddef:
            if vid in lid2viddef[lid]:
                sys.stderr.write(
                    "ERROR duplicate LIDVID " + lid + "::" + vid + "\n  "
                    + aFile + "\n  " + lid2viddef[lid][vid] + "\n")
                continue
        else: lid2viddef[lid] = {}
        lid2viddef[lid][vid] = {}
        lid2viddef[lid][vid] = aFile;   # this populates lid2viddef{}{}
    ...
    for lid in sorted(lid2viddef.iterkeys()):
        for vid in sorted(lid2viddef[lid].iterkeys(), reverse=True):
            ...
            for lidRef in lidTree[lv]:
                if lidRef.text not in lid2refs: lid2refs[lidRef.text] = []
            ...
            if ignoreOld: break     # quit the vid loop after biggest vid
...
if __name__ == "__main__":
    ...
    if verbose: print "___CHECK IF EVERY lid_reference IS DEFINED___"
    for lid in sorted(lid2refs.iterkeys()):
        if lid not in lid2viddef:
            msg = "WARNING " + str(len(lid2refs[lid])) + " lid_references to absent LID " + str(lid)
            print msg
            log_arr.append(msg)
            if verbose:
                for rFile in lid2refs[lid]: print " ", rFile
    if verbose: print "___CHECK IF EVERY lidvid_reference IS DEFINED___"
    for lidvid in sorted(lidvid2refs.iterkeys()):
        lid,vid = lidvid.split("::")
        if lid in lid2viddef:
            if vid in lid2viddef[lid]: continue     # all good
            else:
                msg = "WARNING " + str(len(lidvid2refs[lidvid])) + " lidvid_references to absent VID " + str(lidvid)
                print msg
                log_arr.append(msg)
        else:
            msg =  "WARNING " + str(len(lidvid2refs[lidvid])) + " lidvid_references to absent LID " + str(lidvid)
            print msg
            log_arr.append(msg)
        if verbose:
            for rFile in lidvid2refs[lidvid]: print "   ", rFile
    ...
    if verbose: print "___CHECK THAT ALL LIDS ARE HIERARCHICAL___"
    ...
    for lid in sorted(lid2viddef.iterkeys()):
        ...
        if bundleLID and m.group(1) == bundleLID:
            ...
            bundleLID = m.group(1)
            if verbose: print "INFO bundle:", bundleLID
            if m.group(3):
                collectionLID = m.group(3)
                if verbose: print "INFO collection: ...:" + collectionLID
            else: collectionLID = None  # happens iff bundle product
