...
PROPNO_PATTERN  = re.compile(r' +ID:? +([0-9]{4})\s*')
...
INFO_PATTERN1   = re.compile(r'.{22} *(GO|GTO)[^ ]* +([1-9])\s*')
INFO_HEADER2    = re.compile(r'2\.  *Proposal For +3\. *Cycle\s*')
INFO_PATTERN2   = re.compile(r' *(GO|GTO)[^ ]* +([1-9])\s*')
INFO_HEADER3    = re.compile(r'2\. *Scientific Category +3\. *Proposal For +4\. *Proposal Type\s*')
INFO_PATTERN3   = re.compile(r'.{22} *(GO|GTO).*')

CYCLE_PATTERN   = re.compile(r'.*CYCLE ([0-9]).*')

# Authors are always one PI followed by zero or more CoI lines
AUTHOR_PATTERN1 = re.compile(' *PI?i?:? +(.*?)($|  .*)')
AUTHOR_PATTERN2 = re.compile(' *CoI?n?:? +(.*?)   .*')

# The title is always after this line
TITLE_HEADER    = re.compile('1\. *Proposal Title:\s*')
...
# instead.
YEAR1_PATTERN   = re.compile(' *Check-in Date: .*?-.*?-([0-9]{4})\s*')
YEAR2_PATTERN   = re.compile('.* [01][0-9]/[0-3][0-9]/([0-9]{2}) .*\[  2\]\s*')
YEAR3_PATTERN   = re.compile(' *Report Date: [0-9]{2}-...-([0-9]{2})[^0-9].*')

# This pattern matches any year used in a timing constraint, where the format is
# dd-MON-yy.
REQ_PATTERN     = re.compile('.*[0-9]{1,2}-(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)-([0-9]{2})[^0-9].*')
...
def Citation_Information_from_pro(filename):
    ...
    def fix_title(title):
        ...
        for test in ('a', 'an', 'the', 'for', 'and', 'or', 'at', 'by', 'from',
                     'of', 'on', 'to', 'with'):
            title = title.replace(' ' + test.capitalize() + ' ',
                                  ' ' + test + ' ')
