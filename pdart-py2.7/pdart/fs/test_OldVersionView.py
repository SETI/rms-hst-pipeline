...
_LIDVID_B4 = LIDVID("urn:nasa:pds:hst_00000::4.1")

_LIDVID_C1 = LIDVID("urn:nasa:pds:hst_00000:data_xxx_raw::1.1")
_LIDVID_C2 = LIDVID("urn:nasa:pds:hst_00000:data_xxx_raw::2.1")
_LIDVID_C3 = LIDVID("urn:nasa:pds:hst_00000:data_xxx_raw::3.1")

_LIDVID_P1 = LIDVID("urn:nasa:pds:hst_00000:data_xxx_raw:u2q9xx01j_raw::1.1")
_LIDVID_P2 = LIDVID("urn:nasa:pds:hst_00000:data_xxx_raw:u2q9xx01j_raw::2.1")


class TestOldVersionView(unittest.TestCase):
    def setUp(self):
        # type: () -> None
        memory_fs = MemoryFS()
        ...
        self.versioned_fs.make_lidvid_directories(_LIDVID_B4)

        self.versioned_fs.make_lidvid_directories(_LIDVID_C1)
        self.versioned_fs.make_lidvid_directories(_LIDVID_C2)
        self.versioned_fs.add_subcomponent(_LIDVID_B4, _LIDVID_C3)

        self.versioned_fs.make_lidvid_directories(_LIDVID_P1)
        self.versioned_fs.add_subcomponent(_LIDVID_C3, _LIDVID_P2)

        self.version_view = OldVersionView(_LIDVID_B4, self.versioned_fs)
    ...
    def test_creation(self):
        ...
        self.assertEqual(self.version_view._version_id, u"4.1")
        with self.assertRaises(Exception):
            OldVersionView(LIDVID("urn:nasa:pds:hst_00000::666.333"), self.versioned_fs)
    ...
    def test_bundle_dir(self):
        ...
        self.assertEqual([_COLLECTION_ID], self.version_view.listdir(BUNDLE_DIR))

        # test that collections appear
        self.assertTrue(
            self.version_view.exists(join(ROOT, _BUNDLE_ID, _COLLECTION_ID))
        )

        # test that files don't appear when wrong version
        self.versioned_fs.touch(join(ROOT, _BUNDLE_ID, u"v$2.1", u"bundle.xml"))
        self.assertFalse(
            self.version_view.exists(join(ROOT, _BUNDLE_ID, u"bundle.xml"))
        )

        # test that files do appear when right version
        self.versioned_fs.touch(join(ROOT, _BUNDLE_ID, u"v$4.1", u"bundle.xml"))
        self.assertTrue(self.version_view.exists(join(ROOT, _BUNDLE_ID, u"bundle.xml")))
    ...
    def test_collection_dir(self):
        ...
        self.assertEqual([_PRODUCT_ID], self.version_view.listdir(COLLECTION_DIR))

        # test that files don't appear when wrong version
        self.versioned_fs.touch(join(COLLECTION_DIR, u"v$2.1", u"collection.xml"))
        self.assertFalse(
            self.version_view.exists(join(COLLECTION_DIR, u"collection.xml"))
        )
        # test that files do appear when right version
        self.versioned_fs.touch(join(COLLECTION_DIR, u"v$3.1", u"collection.xml"))
        self.assertTrue(
            self.version_view.exists(join(COLLECTION_DIR, u"collection.xml"))
        )
    ...
    def test_product_dir(self):
        ...
        # test that files don't appear when wrong version
        self.versioned_fs.touch(join(PRODUCT_DIR, u"v$1.1", u"product.xml"))
        self.assertFalse(self.version_view.exists(join(PRODUCT_DIR, u"product.xml")))

        # test that files do appear when right version
        self.versioned_fs.touch(join(PRODUCT_DIR, u"v$2.1", u"product.xml"))
        self.assertTrue(self.version_view.exists(join(PRODUCT_DIR, u"product.xml")))

    def test_directory_to_lid(self):
        # type: () -> None
        with self.assertRaises(AssertionError):
            OldVersionView.directory_to_lid(u"/")
        self.assertEqual(LID("urn:nasa:pds:b"), OldVersionView.directory_to_lid(u"/b"))
        self.assertEqual(
            LID("urn:nasa:pds:b:c"), OldVersionView.directory_to_lid(u"/b/c/")
        )
        self.assertEqual(
            LID("urn:nasa:pds:b:c:p"), OldVersionView.directory_to_lid(u"/b/c/p")
        )

        # what if wrong kind of directory?  i.e., from MultiversionBundleFS?
        with self.assertRaises(AssertionError):
            OldVersionView.directory_to_lid(u"/b/c/v$23.1")
    ...
    def test_lid_to_vid(self):
        ...
        self.versioned_fs.tree()
        self.assertEqual(
            VID("4.1"), self.version_view.lid_to_vid(LID(u"urn:nasa:pds:hst_00000"))
        )
        self.assertEqual(
            VID("3.1"),
            self.version_view.lid_to_vid(LID(u"urn:nasa:pds:hst_00000:data_xxx_raw")),
        )
        self.assertEqual(
            VID("2.1"),
