...
# consistent, all referring to the same body. This can be used as an internal consistency
# check. For example,
...
#
# The reason it returns a list is for possible cases of associated objects, such as a
...
STANDARD_BODY_TYPES = ('Star', 'Planet', 'Satellite', 'Ring', 'Plasma Stream')
SUPPORTED_TYPES = STANDARD_BODY_TYPES + MINOR_PLANET_TYPES

##########################################################################################
# STANDARD_BODY_INFO
#   This is a maintained list of information about Solar System bodies. This list excludes
# small bodies unless they are in some way exceptional.
#
...
#   number          for satellites, the satellite number; otherwise, the MPC number if it
#                   it exists; otherwise, zero.
#   naif_id         NAIF body ID.
#   target_type     'Planet', 'Ring', Satellite', etc. This must be one of the standard
#                   values for the type attribute of a
#                   Target_Identification object.
#   parent          for satellites and rings, the name of the body they orbit; is empty
#                   for Sun-orbiting bodies. The parent body must also appear in this
#                   list.
#   designations    a list of zero or more technical designations for the body. These will
#                   appear as alternate_designations in the Target_Identification object.
##########################################################################################
...
STANDARD_BODY_INFO = [
    ...
    ('Themisto'   , 18,   518, 'Satellite', 'Jupiter', ['S/1975 J 1',
                                                        'S/2000 J 1']),
    ...
    ('Nix'     ,      2, 902, 'Satellite'   , 'Pluto', ['S/2005 $ 2',
                                                        'S/2005 P 2', 'P2']),
    ('Hydra'   ,      3, 903, 'Satellite'   , 'Pluto', ['S/2005 $ 1',
                                                        'S/2005 P 1', 'P3']),
    ('Kerberos',      4, 904, 'Satellite'   , 'Pluto', ['S/2011 $ 1',
                                                        'S/2011 P 1', 'P4']),
    ('Styx'    ,      5, 905, 'Satellite'   , 'Pluto', ['S/2012 $ 1',
                                                        'S/2012 P 1', 'P5']),
...
class StandardBodyInfo(object):
    ...
    def __init__(self, name, number, naif_id, body_type, parent,
                       designations=[]):
    ...
    def minor_planet_info(self):
        ...
        if (not self.minor_planet_info_filled and self.body_type in
                                                  MINOR_PLANET_TYPES):
            if self.designations:
                mp = minor_planets.MinorPlanetInfo(self.body_type,
                                                   self.number,
                                                   self.designations[0],
                                                   self.name,
                                                   self.designations[1:])
    ...
    def __str__(self):
        return ('StandardBodyInfo(' + self.name         + '|' +
                                      str(self.number)  + '|' +
                                      str(self.naif_id) + '|' +
                                      self.body_type    + '|' +
                                      self.parent       + '|' +
    ...
    def __eq__(self, other):
        ...
        if self.designations and other.designations:
            return (self.parent == other.parent and
                    self.designations == other.designations)
    ...
    def copy(self, deep=False):
        ...
        clone = StandardBodyInfo(self.name, self.number, self.naif_id,
                                 self.body_type, self.parent, self.designations)
    ...
    def full_names(self):
        ...
        if self.body_type in ('Star', 'Planet', 'Ring', 'Plasma Stream'):
            ...
            if parent.body_type == 'Planet':
                ...
                if self.number:
                    names += [f'{parent.name} {xx}',
                              f'{parent.name[0]}{self.number}']

            else:
                names = []
                if self.name:
                    names += [f'{parent.number} {parent.name} {xx} {self.name}',
    ...
    def target_identifications(self):
        ...
        if self.body_type in ('Star', 'Plasma Stream'):
            return [(self.name, [], self.body_type, [], self.lid())]

        if self.body_type in ('Planet'):
        ...
        if self.body_type == 'Ring':
            return [(self.name, [], 'Ring',
                     [f'Rings of: {self.parent_info.full_names()[0]}',
        ...
        if self.body_type == 'Satellite':
            ...
            return [(names[0], names[1:], 'Satellite',
                     [f'NAIF ID: {self.naif_id}',
...
##########################################################################################
# Create a dictionary from the STANDARD_BODY_INFO list. Key it by every conceivable
# string.
##########################################################################################
...
for (name, number, naif_id,
     target_type, parent, designations) in STANDARD_BODY_INFO:

    info = StandardBodyInfo(name, number, naif_id, target_type, parent,
                            designations)
