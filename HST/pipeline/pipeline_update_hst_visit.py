...
def setup_logger(args, argv=None):
    ...
    logger = pdslogger.PdsLogger(
        f'pds.hst.update-hst-visit-{proposal_id}-visit_{visit}'
    )
    if not args.quiet:
    ...
    logger.open('update-hst-visit ' + ' '.join(cmd_args), limits=limits)
    return logger


def main(argv=None):
    """Perform the update_hst_visit pipeline task."""
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                f'update_hst_visit failed for proposal id: {formatted_proposal_id}, '
                f'visit: {visit}'
            )
            raise
