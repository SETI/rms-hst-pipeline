...
def get_target_path(args):
    """Return the directory containing FITS files to label."""
...
def setup_logger(args, argv=None):
    ...
    logger.open('label-hst-products ' + ' '.join(cmd_args), limits=limits)
    return logger


def main(argv=None):
    """Perform the label_hst_products pipeline task."""
