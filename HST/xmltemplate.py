...
##########################################################################################
# IMPORTANT: Update the XMLTEMPLATE_VERSION_ID below whenever the behavior of this module
# is changed!
#
# If the change will result in material changes to the content of a label generated by the
# previous version of XmlTemplate.write(), increment the first part of the version number
...
class XmlTemplate(object):
    ...
    1. Create a template object by calling the XmlTemplate constructor to read a template
       file:
            template = XmlTemplate(template_file_path)
    2. Create a dictionary that contains the parameter values to use inside the label.
    3. Construct the label as follows:
            template.write(dictionary, xml_label_file)
    This will create a new XML label of the given name, using the values in the given
    dictionary. Once the template has been constructed, steps 2 and 3 can be repeated any
    number of times.

    SUBSTITUTIONS

    A template file will look generally like an XML file, except for certain embedded
    ...
    The following pre-defined functions can be used inside any expression in the template.
        ...
        NOESCAPE(text):
            ...
            suitable for embedding in an XML label. For example, ">" inside a string will
            be replaced by "&gt;". This function prevents the returned text from being
            escaped, allowing it to contain literal XML.
    ...
    You can include one or more repetitions of the same XML using $FOR and $END_FOR
    ...
    You can use the $NOTE and $END_NOTE headers to embed any arbitrary notes into the
    ...
    def __init__(self, filename, *, content='', logger=None):
        ...
        logger = logger or XmlTemplate.GLOBAL_LOGGER
        ...
        try:
            ...
            # etc.
            parts = XmlTemplate.HEADER_PATTERN.split(content)
            ...
            # Each call to XmlBlock.new_block pops one or more items off top of the deque;
            # the loop repeats until no sections are left.
            self.blocks = deque()
            while sections:
                # Each call to XmlBlock.new_block takes as many sections off the deque as
                ...
                # correctly.
                self.blocks.append(XmlBlock.new_block(sections, self))
    ...
    def set_logger(logger=None):
        ...
        if logger:
            XmlTemplate.GLOBAL_LOGGER = logger
        else:
            XmlTemplate.GLOBAL_LOGGER = pdslogger.NullLogger()
    ...
    def generate(self, dictionary, label_path='', *,
                       ...
                       logger = None,
                       _state = None):
        """Generate the content of one XML label based on the template and dictionary.
        ...
        Input:
            ...
            _state              an optional LabelState object to override the other input
                                parameters.
        """
        ...
        if _state:
            ...
            state = LabelState(dictionary, label_path,
    ...
    def write(self, dictionary, label_path, *,
                    ...
                    raise_exceptions = False,
                    logger = None):
        """Write one XML label based on the template, dictionary, and output filename.
        ...
        Input:
            ...
            state               an optional LabelState object containing the default input
                                parameters.
        """
    ...
    def DATETIME(value, offset=0, digits=None):
        ...
        return XmlTemplate._DATETIME(value, offset, digits, date_type='YMD')
    ...
    def DATETIME_DOY(value, offset=0, digits=None):
        ...
        return XmlTemplate._DATETIME(value, offset, digits, date_type='YD')
    ...
    def DAYSECS(value):
        ...
        if isinstance(value, numbers.Real):
            return XmlTemplate._DATETIME(value, 0, None, date_type='SEC')
    ...
    @staticmethod
    def VERSION_ID():
        """Return the XmlTemplate version ID."""

        return XMLTEMPLATE_VERSION_ID
...
PREDEFINED_FUNCTIONS['FILE_RECORDS'] = XmlTemplate.FILE_RECORDS
PREDEFINED_FUNCTIONS['FILE_ZULU'   ] = XmlTemplate.FILE_ZULU
PREDEFINED_FUNCTIONS['NOESCAPE'    ] = XmlTemplate.NOESCAPE
PREDEFINED_FUNCTIONS['RAISE'       ] = XmlTemplate.RAISE
PREDEFINED_FUNCTIONS['REPLACE_NA'  ] = XmlTemplate.REPLACE_NA
PREDEFINED_FUNCTIONS['REPLACE_UNK' ] = XmlTemplate.REPLACE_UNK
PREDEFINED_FUNCTIONS['VERSION_ID'  ] = XmlTemplate.VERSION_ID
PREDEFINED_FUNCTIONS['WRAP'        ] = XmlTemplate.WRAP
...
class LabelState(object):
    ...
    def __init__(self, dictionary, label_path='', *,
        ...
        self.logger = logger or XmlTemplate.GLOBAL_LOGGER
    ...
    def copy(self):
        return LabelState(self.global_dict, self.label_path, self.terminator,
                          self.raise_exceptions, self.logger)

##########################################################################################
# XmlBlock class and subclasses
##########################################################################################

class XmlBlock(object):
    """XmlBlock is an abstract class that describes a hierarchical section of the XML
    template, beginning with a header. There are individual subclasses to support these
    different types of headers:
        XmlForBlock   for $FOR
        XmlIfBlock    for $IF and $ELSE_IF
        XmlElseBlock  for $ELSE
        XmlNoteBlock  for $NOTE
        XmlOnceBlock  for $END_FOR, $END_IF, $END_NOTE, and any other section of the
                      template for which what follows is included exactly once.

    Each XmlBlock always represents a logically complete section of the template, from one
    ...
    $END_FOR will be described by one XmlForBlock. Every XmlBlock object contains a
    "sub_block" attribute, which is a deque of all the XmlBlocks embedded within it. In
    this case, the sub_blocks attribute will contain a single XmlIfBlock, which in turn
    will contain a single XmlElseBlock.

    Each XmlBlock also has a "body" attribute, which represents the template text between
    ...
    The constructor for each XmlBlock subclass takes a single deque of Sequence objects as
    input. As a side-effect, it removes one or more items from the front of the deque
    until its definition, including any nested XmlBlocks, is complete. The constructor
    handles any nested XmlBlocks within it by calling the constructor recursively and
    saving the results of each recursive call in its sub_blocks attribute.

    Each XmlBlock subclass has its own execute() method. This method contains the logic
    that determines whether (for XmlIfBlocks and XmlElseBlocks) or how many times (for
    XmlForBlocks) its body and sub_blocks are written into the label file. Nesting is
    handled by having each XmlBlock call the execute method of the XmlBlocks nested
    within it.
    """
    ...
    def new_block(sections, template):
        """Construct an XmlBlock subclass based on a deque of Section tuples (header, arg,
        ...
        if header.startswith('$ONCE'):
            return XmlOnceBlock(sections, template)
        if header.startswith('$NOTE'):
            return XmlNoteBlock(sections, template)
        if header == '$FOR':
            return XmlForBlock(sections, template)
        if header == '$IF':
            return XmlIfBlock(sections, template)
        ...
        if header in XmlBlock.ELSE_HEADERS:
    ...
    def preprocess_body(self):
        """Preprocess body text from the XML template by locating all of the embedded
        ...
        # new_parts is a deque of values that alternates between XML substrings and tuples
        ...
        for k,part in enumerate(parts):
            ...
            # Even-numbered items are literal XML
            if k % 2 == 0:
            ...
            # Odd-numbered are expressions, possibly with a name
                ...
                # Look for a name
                match = XmlBlock.NAMED_PATTERN.fullmatch(part)
                if match:
    ...
    def execute_body(self, state):
        """Generate the XML text defined by this body, using the given dictionaries to
    ...
    def execute(self, state):
        """Evaluate this block of XML, using the dictionaries to fill in the blanks. The
        ...
        raising exceptions. Each error also increments the XmlTemplate's ERROR_COUNT.
        """
...
class XmlOnceBlock(XmlBlock):
    ...
    def __init__(self, sections, template):
        ...
        match = XmlOnceBlock.PATTERN.fullmatch(arg)
        if match:
    ...
    def execute(self, state):
        """Evaluate this block of XML, using the dictionaries to fill in the blanks.
        ...
        # exactly once
        return XmlBlock.execute(self, state)

################################################

class XmlNoteBlock(XmlBlock):
    ...
    def __init__(self, sections, template):
        ...
        while sections and sections[0].header != '$END_NOTE':
            self.sub_blocks.append(XmlBlock.new_block(sections, template))
...
class XmlForBlock(XmlBlock):
    ...
    def __init__(self, sections, template):
        ...
        for pattern in (XmlForBlock.PATTERN1, XmlForBlock.PATTERN2, XmlForBlock.PATTERN3):
    ...
    def execute(self, state):
        ...
        for k,item in enumerate(iterator):
            ...
            results += XmlBlock.execute(self, state)

        return results

################################################

class XmlIfBlock(XmlBlock):
    ...
    def __init__(self, sections, template):
        ...
        match = XmlIfBlock.PATTERN.fullmatch(arg)
        if match:
            (self.name, self.arg) = match.groups()
        ...
        self.sub_blocks = deque()
        while sections and sections[0].header not in XmlBlock.ELSE_HEADERS:
            self.sub_blocks.append(XmlBlock.new_block(sections, template))
        ...
        if sections[0].header == '$ELSE_IF':
            self.else_if_block = XmlIfBlock(sections, template)
            return

        # Handle $ELSE
        if sections[0].header == '$ELSE':
            self.else_block = XmlElseBlock(sections, template)
            return
...
def test_substitutions():
    ...
    T = XmlTemplate('t.xml', content='<instrument_id>$INSTRUMENT_ID$</instrument_id>\n')
    D = {'INSTRUMENT_ID': 'ISSWA'}
    V = '<instrument_id>ISSWA</instrument_id>\n'
    assert T.generate(D) == V

    T = XmlTemplate('t.xml', content='<f>$"Narrow" if INST == "ISSNA" else "Wide"$</f>\n')
    D = {'INST': 'ISSWA'}
    V = '<f>Wide</f>\n'
    assert T.generate(D) == V
    ...
    T = XmlTemplate('t.xml', content='<dollar>$$</dollar>\n')
    V = '<dollar>$</dollar>\n'
    assert T.generate({}) == V

    T = XmlTemplate('t.xml', content='<gt>$greater_than$</gt>\n')
    D = {'greater_than': '>'}
    V = '<gt>&gt;</gt>\n'
    assert T.generate(D) == V
...
def test_predefined():
    ...
    # BASENAME
    T = XmlTemplate('t.xml', content='<a>$BASENAME(path)$</a>\n')
    D = {'path': 'a/b/c.txt'}
    V = '<a>c.txt</a>\n'
    assert T.generate(D) == V

    # BOOL
    T = XmlTemplate('t.xml', content='<a>$BOOL(test)$</a>\n')
    D = {'test': 'whatever'}
    V = '<a>true</a>\n'
    assert T.generate(D) == V
    ...
    T = XmlTemplate('t.xml', content='<a>$BOOL(test, true="YES")$</a>\n')
    D = {'test': 'whatever'}
    V = '<a>YES</a>\n'
    assert T.generate(D) == V
    ...
    T = XmlTemplate('t.xml', content='<a>$BOOL(test, true="YES", false="NO")$</a>\n')
    D = {'test': ''}
    V = '<a>NO</a>\n'
    assert T.generate(D) == V

    # CURRENT_ZULU
    T = XmlTemplate('t.xml', content='<a>today=$CURRENT_ZULU()$</a>\n')
    D = {'path': 'a/b/c.txt'}
    V = re.compile(r'<a>today=202\d-\d\d-\d\dT\d\d:\d\d:\d\dZ</a>\n')
    assert V.fullmatch(T.generate(D))

    # DATETIME
    T = XmlTemplate('t.xml', content='<a>$DATETIME(date)$</a>\n')
    ...
    T = XmlTemplate('t.xml', content='<a>$DATETIME(date,-43200)$</a>\n')
    D = {'date': '2000-001T12:34:56'}
    V = '<a>2000-01-01T00:34:56Z</a>\n'
    assert T.generate(D) == V

    T = XmlTemplate('t.xml', content='<a>$DATETIME(date,-43200,digits=0)$</a>\n')
    D = {'date': '2000-001T12:34:56'}
    V = '<a>2000-01-01T00:34:56Z</a>\n'
    assert T.generate(D) == V

    T = XmlTemplate('t.xml', content='<a>$DATETIME(date,-43200,digits=1)$</a>\n')
    D = {'date': '2000-001T12:34:56'}
    V = '<a>2000-01-01T00:34:56.0Z</a>\n'
    assert T.generate(D) == V

    # DATETIME_DOY
    T = XmlTemplate('t.xml', content='<a>$DATETIME_DOY(date)$</a>\n')
    D = {'date': '2000-001T12:34:56'}
    ...
    T = XmlTemplate('t.xml', content='<a>$DATETIME_DOY(date,-43200)$</a>\n')
    D = {'date': '2000-001T12:34:56'}
    V = '<a>2000-001T00:34:56Z</a>\n'
    assert T.generate(D) == V

    T = XmlTemplate('t.xml', content='<a>$DATETIME_DOY(date,-43200,digits=0)$</a>\n')
    D = {'date': '2000-001T12:34:56'}
    V = '<a>2000-001T00:34:56Z</a>\n'
    assert T.generate(D) == V

    T = XmlTemplate('t.xml', content='<a>$DATETIME_DOY(date,-43200,digits=2)$</a>\n')
    D = {'date': '2000-001T12:34:56'}
    V = '<a>2000-001T00:34:56.00Z</a>\n'
    assert T.generate(D) == V

    # DAYSECS
    T = XmlTemplate('t.xml', content='<a>$DAYSECS(date)$</a>\n')
    D = {'date': '2000-001T12:34:56'}
    V = '<a>45296</a>\n'
    assert T.generate(D) == V
    ...
    try:
        ...
        # FILE_BYTES
        T = XmlTemplate('t.xml', content='<bytes>$FILE_BYTES(temp)$</bytes>\n')
        D = {'temp': filepath}
        V = '<bytes>80</bytes>\n'
        assert T.generate(D) == V

        # FILE_MD5
        T = XmlTemplate('t.xml', content='<md5>$FILE_MD5(temp)$</md5>\n')
        V = '<md5>8258601701b61fe08312bac0be88ae48</md5>\n'
        assert T.generate(D) == V

        # FILE_RECORDS
        T = XmlTemplate('t.xml', content='<records>$FILE_RECORDS(temp)$</records>\n')
        V = '<records>10</records>\n'
        assert T.generate(D) == V

        # FILE_ZULU
        os.utime(filepath, None)
        T = XmlTemplate('t.xml', content='$FILE_ZULU(temp)$::$CURRENT_ZULU()$\n')
    ...
    # NOESCAPE
    T = XmlTemplate('t.xml', content='<a>$x$</a>' +
    ...
    # RAISE
    T = XmlTemplate('t.xml', content='$RAISE(ValueError,"This is the ValueError")$\n')
    ...
    try:
        ...
        assert False, "This should have raised an exception but didn't"
    except ValueError as e:
        assert str(e) == V[3:-4]
        assert T.ERROR_COUNT == 1

    # REPLACE_NA
    T = XmlTemplate('t.xml', content='<q>$REPLACE_NA(test,"Not applicable")$</q>\n')
    D = {'test': 111}
    V = '<q>111</q>\n'
    assert T.generate(D) == V
    ...
    # REPLACE_UNK
    T = XmlTemplate('t.xml', content='<q>$REPLACE_UNK(test,"Unknown")$</q>\n')
    D = {'test': 111}
    V = '<q>111</q>\n'
    assert T.generate(D) == V
    ...
    # VERSION_ID
    T = XmlTemplate('t.xml', content='<!-- XmlTemplate version $VERSION_ID()$ -->\n')
    V = '<!-- XmlTemplate version ' + XMLTEMPLATE_VERSION_ID + ' -->\n'
    assert T.generate({}) == V

    # WRAP
    T = XmlTemplate('t.xml', content="<a>\n    $WRAP(4,80,lorem_ipsum)$\n</a>\n")
...
def test_headers():
    ...
    # $FOR and $END_FOR
    T = XmlTemplate('t.xml', content="""<a></a>
        $FOR(targets)
        <target_name>$VALUE$ ($naif_ids[INDEX]$)</target_name>
        $END_FOR
        <b></b>\n""")
    ...
    <x>$x$</x>
    $END_IF\n""")
    assert T.generate({'x': 0 }) == '    <x>zero</x>\n'
    assert T.generate({'x': 1.}) == '    <x>one</x>\n'
    assert T.generate({'x': 3 }) == '    <x>3</x>\n'
    assert T.generate({'x': 3.}) == '    <x>3.</x>\n'
    ...
    <x>False ($a$, $b$)</x>
    $END_IF\n""")
    assert T.generate({'x': [1.], 'y': 2}) == '    <x>x is True ([1.0])</x>\n'
    assert T.generate({'x': [],   'y': 2}) == '    <x>y is True ([], 2)</x>\n'
    assert T.generate({'x': [],   'y': 0}) == '    <x>False ([], 0)</x>\n'
...
def test_terminators():
    ...
    T = XmlTemplate('t.xml', content='<a>$A$</a>\n<b>$B$</b>\n<c>$C$</c>\n')
    D = {'A': 1, 'B': 2, 'C': 3}
    V = '<a>1</a>\n<b>2</b>\n<c>3</c>\n'
    assert T.generate(D, terminator=None) == V
    assert T.generate(D, terminator='\n') == V
