...
# must be internally consistent, all referring to the same minor planet. This
# can be used as an internal consistency check. For example,
...
# as fragments of a single comet, that might all simultaneously be targets of a
# given observation. However, this capability is not yet used for minor planets.
#
# For additional internal checks, you can also call the function with explicit
# values for orbital elements a, e and i. If provided, an error will arise if
# any specified orbital element differs from that in the MPC by the specified
# threshhold of accuracy.
#
# For minor planets that were also classified as comets, the returned tuple
# includes the comet names as alternative designations. However, you cannot use
# a key that follows cometary nomenclature; use comets.comet_identifications()
# for that purpose.
#
# NOTE: This module performs real-time queries of the Minor Planet Center. As a
# result, expect a time delay of a few seconds for each identification.
################################################################################
...
#
# This module cannot import comets, because comets imports minor_planets. The
# solution is that, when the comets module initializes, it defines these
# attributes of the minor_planets module.
...
class MinorPlanetInfo(object):
    ...
    def __init__(self, body_type, number, designation, name, alt_designations=[]):
    ...
    def copy(self, deep=False):
        ...
        clone = MinorPlanetInfo(
            self.number, self.designation, self.name, self.alt_designations
        )
    ...
    def full_names(self):
        ...
        if self.number and self.name:
            names += [f"{self.number} {self.name}", f"({self.number}) {self.name}"]
        ...
        if self.number:
            names += [f"Minor Planet {self.number}", f"NAIF ID {self.naif_id}"]
...
def minor_planet_identifications(
    keys, a=0.0, e=0.0, i=0.0, q=0.0, warnings=[], check_comets=True
):
    ...
    for key in keys:
        ...
        if minor_planet_identifications.IGNORE_EXTRA_NAMES:
            warnings.append(
                "WARNING: Malformed minor planet identifier, " + f"ignored: {key}"
            )
            if minor_planet_identifications.DEBUG:
    ...
    for key in keys:
        ...
        if not info:
            if minor_planet_identifications.IGNORE_EXTRA_NAMES:
                warnings.append(
                    "WARNING: Unrecognized MPC identifier, " + f"ignored: {key}"
                )
                if minor_planet_identifications.DEBUG:
        ...
        # Subsequent names must match the first
            ...
            raise ValueError(
                "Inconsistent minor planet designations: " + str(original_keys)
            )
    ...
    if name == "Pluto":
        ...
        mp = MinorPlanetInfo(body_type, number, designations[0], name, designations[1:])
...
def check_elements(a, e, i, q, a0, e0, i0, q0, warnings=[]):
    ...
    if (a, e, i, q) == (0.0, 0.0, 0.0, 0.0):
        return

    # Orbital elements not available
    if (a0, e0, i0, q0) == (0.0, 0.0, 0.0, 0.0):
        warnings.append("WARNING: MPC orbital elements are not available")
        return
    ...
    if i:
        errors.append(abs(i - i0) * math.pi / 180.0)  # absolute, in radians
        values += [(i, i0)]
    ...
    if errors[-1] > THRESHOLD:
        warnings.append(f"WARNING: One orbital element mismatch: {values}")
