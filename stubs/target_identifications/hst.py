...
SPT_REPAIRS = {
    ...
    "5834_2": {
        "TARKEY1": "COMET SHOEMAKER 87O-86XIV",
        # was "SHOEMAKER 87O-85XIV"
        "TARGNAME": "C1987H1 SHOEMAKER",
    },  # was "COMET-SHOEMAKER-1987H1"
    "6736": {
        "TARKEY1": "COMET C/1996 B2 (HYAKUTAKE)",
        "TARGNAME": "C/1996 B2",
    },  # was "COMET B2-NUCLEUS", "B2"
    "7594": {"TARKEY2": "COMET C/1997 BA6"},  # year was 1996
    "8699_2": {
        "TARGNAME": "ASHBROOK-JACKSON",  # was "ASHBROOK"
        "MT_LV1_1": "FILE=",
    },  # revised elements
    ...
    "9678_1": {
        "TARGNAME": "50000 QUAOAR",  # was "OBJECTX"
        "MT_LV1_1": "FILE=",
    },  # revised elements
    ...
    "12537_ANY": {"MT_LV1_1": "STD=EARTH", "MT_LV2_1": "STD=MOON"},  # missing
    "12792_1": {"TARKEY2": "C/2011 W3 LOVEJOY"},  # was "New...comet"
    "14192_1": {
        "TARKEY1": "COMET",  # was "ASTEROID"
        "TARKEY2": "",
    },  # was extraneous info
    ...
    "16077_1": {
        "TARGNAME": "P/2019 LD2",  # was "2019LD2"
    ...
    "15344_30": [
        (
            ...
            "urn:nasa:pds:context:target:" + "trans-neptunian_object.2011_uh413",
        )
    ],
    # The abstract indicates these targets
    "15492_3": (
        minor_planet_identifications("2014 OS393")
...
def tno_survey(spt_lookup):
    ...
    return [
        (
            f"TNO Survey Field HST-{targ_id}",
            [],
            "Trans-Neptunian Object",
            [
...
def hst_target_identifications(spt_lookup):
    ...
    for key in [TARG_ID, PROPOSID, PROPOSID + "_" + TARG_ID]:
        try:
            ...
            if DEBUG:
                print("repair found for key", key)
            break
        except KeyError:
            pass
    ...
    if isinstance(repair, list):
        if DEBUG:
            print("target identified in SPT_REPAIRS")
        return repair
    ...
    if STDs:
        ...
        if len(MT_LVs) > 1 and MT_LVs[1].startswith("TYPE=TORUS"):
            ...
            if DEBUG:
                print(f"TORUS, rad = {rad}, planet = {planet}")

            # Io torus is for planet Jupiter, POLE_LAT missing, and RAD > 150000
            if planet == "JUPITER" and "POLE_LAT" not in MT_LVs[1] and rad > 150000.0:
                if DEBUG:
                    print("Io torus targeting identified")
                return standard_body_identifications("Io Torus")

            # Otherwise, a ring requires RAD greater than the planet radius
            if rad >= PLANET_RADII[planet]:
                if DEBUG:
                    print("Rings targeting identified")
                return standard_body_identifications(planet + " rings")
            else:
                if DEBUG:
                    print("Planet targeting identified")
    ...
    if DEBUG:
        print("votes, TARKEYs =", votes, TARKEYs)
    ...
    if left == "TYPE":
        if right == "ASTEROID":
            ...
            return standard_body_identifications(
                "PLUTO"
    ...
    if get_elements:
        ...
        if DEBUG:
            print("a,q,e,i =", a, q, e, i)

    else:
        a = q = e = i = 0.0
        if DEBUG:
            print("a,q,e,i ignored")

    # Interpret target info
    TARGNAME = spt_lookup["TARGNAME"]

    if DEBUG:
        print("Candidate identifiers =", TARKEYs + [TARGNAME])
    ...
    for text in TARKEYs + [TARGNAME]:
        ...
        if DEBUG:
            print("PREPROCESSOR1:", text)

        text = PREPROCESSOR2.first(text)
        if DEBUG:
            print("PREPROCESSOR2:", text)

        (vote, text) = CLASSIFIER.first(text)
        if DEBUG:
            print("CLASSIFIER:", vote, text)

        votes += vote
        identifiers += [text]

    if DEBUG:
        print("votes, identifiers =", votes, identifiers)

    comet_ids = []
    if "C" in votes:
        for identifier in identifiers:
            if DEBUG:
                print("Comet test for", identifier)

            identifier = COMET_REPAIRS.first(identifier)
            if DEBUG:
                print("COMET_REPAIRS", identifier)

            ids = list(COMET_TRANSLATOR.first(identifier))
            if DEBUG:
                print("COMET_TRANSLATOR", ids)

            comet_ids += [i for i in ids if i]

        if DEBUG:
            print("comet_ids", comet_ids)

    mp_ids = []
    if "M" in votes:
        for identifier in identifiers:
            if DEBUG:
                print("Minor planet test for", identifier)

            identifier = MINOR_PLANET_REPAIRS.first(identifier)
            if DEBUG:
                print("MINOR_PLANET_REPAIRS", identifier)

            ids = list(MINOR_PLANET_TRANSLATOR.first(identifier))
            if DEBUG:
                print("MINOR_PLANET_TRANSLATOR", ids)
    ...
    if comet_ids:
        if DEBUG:
            print("Testing comet_ids", comet_ids)
        try:
            ids = comet_identifications(
                comet_ids,
                a=a,
                e=e,
                i=i,
    ...
    if mp_ids:
        if DEBUG:
            print("Testing mp_ids", mp_ids)
        try:
            ids = minor_planet_identifications(
                mp_ids, a=a, e=e, i=i, q=q, warnings=mp_warnings
            )
            if ids:
